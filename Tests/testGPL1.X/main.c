...
{
    if(CCP1STATbits.ICBNE == 1) {
        int a = CCP1BUF;    // erster Bufferwert (FIFO) - kleinerer Wert
        a = CCP1BUF;    // zweiter Bufferwert (FIFO) - größerer Wert
        //CCP1TMR = 0;               // reset timer
        if(CCP1STATbits.ICBNE == 0) {   // wenn beide Bufferwerte ausgelesen wurden, Flag ICBNE ist 0
